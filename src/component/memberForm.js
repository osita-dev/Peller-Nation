...
const MembershipForm = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: files ? files[0] : value, // Update formData with file or value
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success && result.userId) {
        setUserId(result.userId); // Store userId
        ...
        const paymentResponse = await fetch("https://pellerserver.onrender.com/generate-payment-link", {
          ...
          body: JSON.stringify({
            ...
            amount: formData.badge, // Pass the selected amount for payment
          }),
        });
        ...
        if (paymentData.paymentLink) {
          window.location.href = paymentData.paymentLink; // Redirect to Paystack payment page
        } else {
